...
| `moon test` | verified in current local environment | verified | verified | not separately re-run | not run |
...
- The root `moon test` line now has fresh local evidence on `native`, `js`, and `wasm` in this environment.
